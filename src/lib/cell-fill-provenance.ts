...
import {
  combineScheduleStates,
  scheduleStateFromColor,
...
export function resolveSourceCellFills(
  ...
  rows.forEach((_, rowIndex) => {
    ...
    for (const [columnKey, range] of layout.columnRanges) {
      // Uma cor por célula final: a da coluna que dá nome a ela. As demais
      // células da faixa mesclada são lidas separadamente por quem precisa
      // delas (ver `resolveScheduleFillStates`).
      const address = XLSX.utils.encode_cell({ r: sheetRow, c: range.start });
...
/**
 * Estado de cronograma que só existe como COR na planilha de origem. Um
 * cronograma de calibração costuma deixar as células de mês sem texto nenhum
 * e pintar o andamento (a própria planilha traz a legenda: azul = programado,
 * verde = realizado, amarelo = reprogramado, vermelho = não realizado). Sem
 * ler a cor, o cronograma visual fica inteiro vazio mesmo com a planilha
 * toda preenchida.
 *
 * Cada célula-mês pode cobrir mais de uma coluna da grade original (uma para
 * o previsto, outra para o ocorrido); o desfecho prevalece sobre a intenção
 * quando as duas estão pintadas.
 */
export function resolveScheduleFillStates(
  rows: Row[],
  columns: Column[],
  diagnostics: ImportDiagnostics | undefined,
  audit: ImportAudit | undefined,
  ...
  const results: ScheduleFillState[] = [];
  rows.forEach((_, rowIndex) => {
    const sheetRow = layout.origins[rowIndex];
    if (sheetRow === undefined || sheetRow < 0) return;
    for (const [columnKey, range] of layout.columnRanges) {
      const states: ScheduleCellState[] = [];
      for (let column = range.start; column <= range.end; column++) {
        const address = XLSX.utils.encode_cell({ r: sheetRow, c: column });
        const color = layout.fillByAddress.get(address);
        const state = color ? scheduleStateFromColor(color) : null;
        if (state) states.push(state);
      }
      const state = combineScheduleStates(states);
      if (state) results.push({ rowIndex, columnKey, state });
    }
  });
  return results;
}
