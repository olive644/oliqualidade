...
| Extração do diálogo de junção (`useJoinSheetDialog`) é puramente estrutural, mas ainda assim reduziu a margem de bundle de ~415,3 para ~418,6 KiB | mesma fragilidade de "fachada de chunk compartilhado" das duas linhas acima — mover código entre arquivos de primeira-parte muda o resultado do chunking mesmo sem nova lógica | margem crítica agora (1,4 KiB contra 420 KiB); próxima etapa da extração do Dashboard (seção 51 do audit) precisa de decisão do usuário: aumentar o limite do orçamento ou investir em `rollup-plugin-visualizer` antes de continuar |
