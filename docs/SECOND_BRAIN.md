...
| Varredura de segurança do código na CI | job `static-analysis` em `.github/workflows/application.yml`: Semgrep OSS com versão fixa (`semgrep==1.174.0`) e os pacotes `p/typescript`, `p/react` e `p/nodejs`, sem conta e com `--metrics=off`; segredos ficam com o gitleaks do job `secret-scan`, que varre o histórico | sem teste local: o Semgrep não roda no Windows e o ambiente não tem Python — a primeira execução é a da própria CI; ver [[CURRENT_STATE_AUDIT#135. Semgrep no lugar do CodeQL: análise estática num repositório privado]] |
...
- `v0.10.0-beta.3` (Semgrep no lugar do CodeQL desligado com a mudança para
  repositório privado). Ver
  [[CURRENT_STATE_AUDIT#135. Semgrep no lugar do CodeQL: análise estática num repositório privado]].
