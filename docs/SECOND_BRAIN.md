...
| Identificar as séries do gráfico de área sem passar o mouse | `ChartSeriesLegend` (`widget-support.tsx`), alimentada por `areaLegendItems` em `chart-widget-body.tsx`; desenha o traço real de cada série, então a distinção não depende só de cor | verificação manual no navegador — ver seção 119 do audit |
