...
| Coordenador do caminho progressivo de OOXML, escrito e ainda desligado (ligar é a recomendação) | `ooxml-progressive-import.ts`: descompacta uma vez, lê a grade, monta workbook mínimo (`minimalWorksheetForOoxmlGrid`), anexa recursos com `attachWorkbookFeatures`, normaliza com `sheetsWithData(wb, { gridFor })` num buffer, e recusa (`ProgressiveImportFallback`) o arquivo inteiro se alguma aba saiu dividida em seções (nome com `" · "`) antes de emitir qualquer coisa; não roda `XLSX.read` nem a verificação cruzada | `ooxml-progressive-import.test.ts` (paridade sintética, recusa determinística contra divisão em seções, e contra o corpus real: 1 arquivo recusado, piso de 79 abas idênticas nos demais), `ooxml-progressive-benchmark.test.ts` (53% menos pico, 38% mais rápido pelo coordenador inteiro); `PROGRESSIVE_IMPORT_SUPPORT.ooxml` continua `false` — ver [[CURRENT_STATE_AUDIT#170. A divisão em seções não foi alinhada, foi contornada: recusa por nome]] |
...
| Decidir se um widget salvo ainda serve para a planilha | `widgetCompatible` (`src/lib/dashboard.ts`), consumida por `repairInvalidWidgets` no carregamento; tipo sem ramo próprio cai na regra geral, que exige `groupKey` **e** `valueKey` | histograma e dispersão não têm `groupKey` por definição e precisam de ramo próprio, senão são destruídos ao recarregar; `dashboard-widget-survival.test.ts` compara identidade (id, título, coluna, faixas) e não presença do tipo — ver [[CURRENT_STATE_AUDIT#156. O histograma configurado era destruído ao recarregar o painel]] |
...
| Hover troca sem quadro vazio entre barras | sair da barra A ocorre antes de entrar na B; limpar A imediatamente e atrasar B fazia todas voltarem à opacidade normal por 90 ms, produzindo piscar | a limpeza também é agendada e é cancelada pela entrada seguinte; a seleção anterior fica visível até a nova estar pronta — ver [[CURRENT_STATE_AUDIT#168. O amortecimento do hover ainda criava um quadro de piscar entre barras]] |
| Série do histograma é memoizada | hover é estado local de inspeção, mas valores, faixas e objetos da série eram recriados a cada renderização e entregues como `data` novo ao Recharts | o pipeline depende apenas de dados, coluna e quantidade de faixas; hover não remonta as barras — ver [[CURRENT_STATE_AUDIT#168. O amortecimento do hover ainda criava um quadro de piscar entre barras]] |
| Barra não usa entrada por opacidade | uma barra SVG remendada ou remontada voltava a `opacity: 0`, tornando uma atualização comum visível como sumiço | permanecem transições de destaque, sem animação que esconda o dado — ver [[CURRENT_STATE_AUDIT#168. O amortecimento do hover ainda criava um quadro de piscar entre barras]] |
| ------------------------------------------------------------ | ----------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- |
| O índice de hover de um gráfico espera o ponteiro sossegar | atualizar a cada forma atravessada reconstrói a árvore inteira do gráfico, e medido isso era 83% do custo de inspecionar: 14 tarefas longas e 2.883 ms contra 2 e 504 ms sem hover nenhum | 90 ms de espera na entrada, nenhuma na saída; atravessar não é inspecionar, e atraso na saída apareceria como destaque preso — ver [[CURRENT_STATE_AUDIT#167. A troca de `both` por `backwards` fez a página inteira ganhar rolagem]] |
| Memoizar o array de série e amortecer o hover só valem juntos | a memoização sozinha não mudou nada, porque com tudo sendo reconstruído a cada hover a identidade estável do array não ajudava; com o amortecimento no lugar ela derruba o restante quase pela metade | medir uma otimização isolada pode dizer "não serve" quando o que falta é a outra metade |
...
- `v0.10.0-beta.14` (histograma e dispersão param de perder a configuração ao
...
- `v0.10.0-beta.17` (uma data inválida deixa de cancelar a verificação da
