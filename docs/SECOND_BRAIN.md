...
| Coordenador do caminho progressivo de OOXML, escrito e ainda desligado | `ooxml-progressive-import.ts`: descompacta uma vez, lê a grade, monta workbook mínimo (`minimalWorksheetForOoxmlGrid`), anexa recursos com `attachWorkbookFeatures` e normaliza com `sheetsWithData(wb, { gridFor })`; não roda `XLSX.read` nem a verificação cruzada; `ProgressiveImportFallback` mora em `workbook-reading-engine.ts`, reexportada de `csv-progressive-import.ts` por compatibilidade | `ooxml-progressive-import.test.ts` (paridade sintética e contra o corpus, piso de 87 abas idênticas), `ooxml-progressive-benchmark.test.ts` (53% menos pico, 38% mais rápido pelo coordenador inteiro); `PROGRESSIVE_IMPORT_SUPPORT.ooxml` continua `false` — ver [[CURRENT_STATE_AUDIT#169. O coordenador do caminho progressivo de OOXML, ainda desligado]] |
