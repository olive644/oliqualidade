...
| Assinar cookie sem estado no servidor | `lib/signed-cookie.ts` (HMAC, prazo, marca do `user-agent`), usado por `chat-session.ts` e por `human-check.ts` | extraído de `chat-session.ts` quando a prova humana passou a precisar do mesmo: duplicar código de assinatura deixaria um dos dois para trás em silêncio; ver [[CURRENT_STATE_AUDIT#137. Limite de requisições compartilhado e verificação Cloudflare Turnstile]] |
