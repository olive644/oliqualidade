...
| Leitura de CSV por streaming                 | `csv-stream.ts`: reconhecimento de codificação em passagem própria, analisador que guarda estado entre pedaços, blocos com um em voo por vez | `csv-stream.test.ts` (corte em toda posição, multibyte partido, equivalência com o SheetJS) — ver [[CURRENT_STATE_AUDIT#147. Streaming de CSV de verdade: o arquivo nunca entra inteiro na memória]] |
